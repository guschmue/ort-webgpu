...
/*!******************!*\
  !*** ./index.js ***!
  \******************/
n.r(r);var e=n(/*! onnxruntime-web/webgpu */"./node_modules/onnxruntime-web/dist/esm/ort.webgpu.min.js");const t=500,a=500,i=1024,s=1024,o={sam_b:[{name:"sam-b-encoder",url:"https://huggingface.co/schmuell/sam-b-fp16/resolve/main/sam_vit_b_01ec64.encoder-fp16.onnx",size:180},{name:"sam-b-decoder",url:"https://huggingface.co/schmuell/sam-b-fp16/resolve/main/sam_vit_b_01ec64.decoder.onnx",size:17}]},u=function(){const e=window.location.search.substring(1);var t={model:"sam_b",provider:"webgpu",device:"gpu",threads:"1"};let n=e.split("&");for(var r=0;r<n.length;r++){let e=n[r].split("=");if(e[0]in t)t[e[0]]=decodeURIComponent(e[1]);else if(e[0].length>0)throw new Error("unknown argument: "+e[0])}return t.threads=parseInt(t.threads),t.local=parseInt(t.local),t}();let l,d,p;var c;e.default.env.wasm.wasmPaths="dist/",e.default.env.wasm.numThreads=u.threads,e.default.env.wasm.proxy=!1;var f,h,m=[],g=[],y=!1;function w(e){document.getElementById("status").innerText+=`\n${e}`}async function $(e){if(0==m.length)return;const[t,n]=[l.width,l.height],r=new OffscreenCanvas(t,n),a=r.getContext("2d"),i=a.getImageData(0,0,t,n),s=new OffscreenCanvas(t,n).getContext("2d");s.drawImage(await createImageBitmap(h),0,0);const o=s.getImageData(0,0,t,n);for(let e=0;e<o.data.length;e+=4)if(o.data[e]>0)for(let t=0;t<4;++t){const n=e+t;i.data[n]=f.data[n]}a.putImageData(i,0,0);const u=document.createElement("a");u.download="image.png",u.href=URL.createObjectURL(await r.convertToBlob()),u.click(),u.remove()}async function v(t,n){let r=l.getContext("2d");if(r.clearRect(0,0,l.width,l.height),l.width=f.width,l.height=f.height,r.putImageData(f,0,0),t.length>0){void 0===c&&await o[u.model][0].sess;const a=await c,l=o[u.model][1].sess,d=function(t,n,r){const a=new e.default.Tensor(new Float32Array(65536),[1,1,256,256]),o=new e.default.Tensor(new Float32Array([0]),[1]),u=new e.default.Tensor(new Float32Array([s,i]),[2]),l=new e.default.Tensor(new Float32Array(n),[1,n.length/2,2]),d=new e.default.Tensor(new Float32Array(r),[1,r.length]);return{image_embeddings:(p=t.image_embeddings,new e.default.Tensor(p.type,Float32Array.from(p.data),p.dims)),point_coords:l,point_labels:d,mask_input:a,has_mask_input:o,orig_im_size:u};var p}(a,t,n),f=performance.now(),m=await l.run(d);p.innerText=`${(performance.now()-f).toFixed(1)}ms`;for(let e=0;e<t.length;e+=2)r.fillStyle="blue",r.fillRect(t[e],t[e+1],10,10);const g=m.masks;h=g.toImageData(),r.globalAlpha=.3,r.drawImage(await createImageBitmap(h),0,0)}}function b(e){const t=l.getBoundingClientRect();return[Math.trunc(e.clientX-t.left),Math.trunc(e.clientY-t.top)]}async function _(e){if(!y)try{y=!0,l.style.cursor="wait";const t=b(e);await v([...m,t[0],t[1]],[...g,1])}finally{l.style.cursor="default",y=!1}}async function x(e){if(!y)try{y=!0,l.style.cursor="wait";const t=b(e),n=1;m.push(t[0]),m.push(t[1]),g.push(n),await v(m,g)}finally{l.style.cursor="default",y=!1}}async function S(n){const r=document.getElementById("encoder_latency");r.innerText="",m=[],g=[],d.disabled=!0,p.innerText="",l.style.cursor="wait",c=void 0;let h=n.width,y=n.height;h>y?h>t&&(y*=t/h,h=t):y>a&&(h*=a/y,y=a),h=Math.round(h),y=Math.round(y),l.width=h,l.height=y;var w=l.getContext("2d");w.drawImage(n,0,0,h,y),f=w.getImageData(0,0,h,y);const $=await e.default.Tensor.fromImage(f,{resizedWidth:i,resizedHeight:s}),v=u.isSlimSam?{pixel_values:$}:{input_image:$},b=await o[u.model][0].sess,_=performance.now();(c=b.run(v)).then((()=>{r.innerText=`${(performance.now()-_).toFixed(1)}ms`,l.style.cursor="default"})),d.disabled=!1}async function E(e,t){try{const n=await caches.open("onnx");let r=await n.match(e);null==r?(await n.add(e),r=await n.match(e),w(`${t} (network)`)):w(`${t} (cached)`);return await r.arrayBuffer()}catch(n){return w(`${t} (network)`),await fetch(e).then((e=>e.arrayBuffer()))}}async function I(){o[u.model];l=document.getElementById("img_canvas"),l.style.cursor="wait",d=document.getElementById("file-in"),p=document.getElementById("decoder_latency"),document.getElementById("clear-button").addEventListener("click",(()=>{v(m=[],g=[])}));let t=document.getElementById("original-image");await async function(t){const n=await caches.open("onnx");let r=0;for(const[e,a]of Object.entries(t))void 0===await n.match(a.url)&&(r+=a.size);w(r>0?`downloading ${r} MB from network ... it might take a while`:"loading...");const a=performance.now();for(const[n,r]of Object.entries(t))try{const t={executionProviders:[u.provider],enableMemPattern:!1,enableCpuMemArena:!1,extra:{session:{disable_prepacking:"1",use_device_allocator_for_initializers:"1",use_ort_model_bytes_directly:"1",use_ort_model_bytes_for_initializers:"1"}}},n=await E(r.url,r.name),a=r.opt||{},i={...t,...a};r.sess=await e.default.InferenceSession.create(n,i)}catch(e){w(`${r.url} failed, ${e}`)}w(`ready, ${(performance.now()-a).toFixed(1)}ms`)}(o[u.model]).then((()=>{l.addEventListener("click",x),l.addEventListener("mousemove",_),document.getElementById("cut-button").addEventListener("click",$),d.onchange=function(e){let n=(e.target||window.event.src).files;if(FileReader&&n&&n.length){let e=new FileReader;e.onload=()=>{t.onload=()=>S(t),t.src=e.result},e.readAsDataURL(n[0])}},S(t)}),(e=>{w(e)}))}document.addEventListener("DOMContentLoaded",(()=>{(async function(){try{return(await navigator.gpu.requestAdapter()).features.has("shader-f16")}catch(e){return!1}})().then((e=>{e?I():w("Your GPU or Browser doesn't support webgpu/f16")}))}))})();
